...
CHECKLIST_STEP_ORDER = [
    ("sorting", "СОРТИРОВКА"),
    ("set_workplace", "СОЗДАЙ РАБОЧЕЕ МЕСТО"),
    ("cleanliness", "СОДЕРЖАНИЕ В ЧИСТОТЕ"),
    ("standardization", "СТАНДАРТИЗАЦИЯ"),
    ("improvement", "СОВЕРШЕНСТВОВАНИЕ"),
]

CHECKLIST_STEP_INDEX = {
    code: idx for idx, (code, _) in enumerate(CHECKLIST_STEP_ORDER, start=1)
}
...
UKM5_API_HOST = _env_value(
    "UKM5_API_HOST",
    "192.168.17.38",
)
UKM5_API_PORT = int(
    _env_value("UKM5_API_PORT", "29017")
)

UKM5_DB_HOST = _env_value(
    "UKM5_DB_HOST",
    "192.168.17.38",
)
UKM5_DB_PORT = int(
    _env_value("UKM5_DB_PORT", "3306")
)
UKM5_DB_NAME = _env_value(
    "UKM5_DB_NAME",
    "srvdata",
)
UKM5_DB_USER = _env_value(
    "UKM5_DB_USER",
    "ukminfo",
)
UKM5_DB_PASSWORD = _env_value(
    "UKM5_DB_PASSWORD",
    "CtHDbCGK.C",
)

UKM5_HTTP_TIMEOUT = int(
    _env_value("UKM5_HTTP_TIMEOUT", "20")
)

# Полный срок ожидания появления результата импорта.
UKM5_VERIFY_TIMEOUT = int(
    _env_value("UKM5_VERIFY_TIMEOUT", "20")
)

UKM5_VERIFY_SLEEP = float(
    _env_value("UKM5_VERIFY_SLEEP", "1.0")
)

# Количество повторных отправок импорта при задержке УКМ-5.
UKM5_SYNC_ATTEMPTS = max(
    1,
    int(_env_value("UKM5_SYNC_ATTEMPTS", "2")),
)

UKM5_CARD_TZ = _env_value(
    "UKM5_CARD_TZ",
    _env_value("ROTATION_TZ", "Asia/Irkutsk"),
)
...
def _ukm5_text(value) -> str:
    """
    Безопасно приводит значение из PyMySQL к строке.
    """
    if value is None:
        return ""
...
def _ukm5_is_active(value) -> bool:
    """
    Нормализует active из TINYINT, BIT, bytes или строки.
    """
    if value is None:
        return False
...
def _poll_ukm5_user_state(
    *,
    ...
    """
    Проверяет результат асинхронного импорта УКМ-5.

    Важно: одинаковое состояние несколько раз подряд не означает,
    что импорт завершился. Поэтому ждём до полного timeout и не
    завершаем проверку преждевременно.
    """
    timeout_value = float(
        timeout_sec
        if timeout_sec is not None
        else UKM5_VERIFY_TIMEOUT
    )
    ...
    expected_password_text = (
        _ukm5_text(expected_password)
        if expected_password is not None
        else None
    )

    expected_active_text = (
        _ukm5_text(expected_active_number)
        if expected_active_number is not None
        else None
    )

    forbidden_active_numbers = {
        _ukm5_text(number)
        for number in (forbidden_active_numbers or set())
        if _ukm5_text(number)
    }
    ...
    while True:
        ...
        current_password = (
            _ukm5_text(user_row.get("password"))
            if user_found
            else ""
        )

        password_ok = (
            expected_password_text is None
            or (
                user_found
                and current_password == expected_password_text
            )
        )

        expected_card_ok = (
            expected_active_text is None
            or expected_active_text in active_numbers
        )

        stale_active_numbers = active_numbers.intersection(
            forbidden_active_numbers
        )
        forbidden_ok = not stale_active_numbers

        ready = (
            user_found
            and password_ok
            and expected_card_ok
            and forbidden_ok
        )
        ...
        if ready:
            logger.info(
                f"[UKM5][VERIFY] ready "
                f"store_id={internal_store_id} "
                f"cashier_id={cashier_id} "
                f"poll={poll_number} "
                f"active_count={len(active_numbers)}"
            )
            return state, True
        ...
        signature = (
            ...
            current_password,
            tuple(sorted(active_numbers)),
        )

        if signature != last_signature:
            logger.info(
                f"[UKM5][VERIFY] state_changed "
                f"store_id={internal_store_id} "
                f"cashier_id={cashier_id} "
                f"poll={poll_number} "
                f"user_found={user_found} "
                f"password_ok={password_ok} "
                f"expected_card_ok={expected_card_ok} "
                f"active_count={len(active_numbers)} "
                f"stale_count={len(stale_active_numbers)}"
            )
        ...
        sleep_sec = min(
            max(0.1, float(UKM5_VERIFY_SLEEP)),
            remaining,
        )
...
def _normalize_selected_services(raw_items: Iterable[str]) -> list[str]:
    ...
    Сохраняет порядок, убирает дубли.
    """
    out = []
    seen = set()

    for raw in raw_items or []:
        db = _normalize_service_key(raw)
        if not db:
            continue
        if db in seen:
            continue
...
def _oracle_resolve_position_for_target_db(cur, source_position: dict | None) -> dict | None:
    """
    Находит соответствующую должность в целевой базе.
    Сопоставление:
    1) по ORAROLE
    2) по ID
    3) по TITLE
    ...
    target_positions = _oracle_fetch_smoffcfg_choices(cur)

    source_id = str(source_position.get("id") or "").strip()
    source_title = (source_position.get("title") or "").strip().lower()
    source_orarole = (source_position.get("orarole") or "").strip().lower()
    ...
    if source_orarole:
        matched = next(
            (
                ...
                if (x.get("orarole") or "").strip().lower() == source_orarole
            ),
            None
        )
    ...
    if not matched and source_id:
        matched = next(
            (
                x for x in target_positions
                if str(x.get("id")) == source_id
            ),
            None
        )

    if not matched and source_title:
        matched = next(
            (
                x for x in target_positions
                if (x.get("title") or "").strip().lower() == source_title
            ),
            None
        )
    ...
    return {
        ...
        "id": int(matched["id"]) if matched and matched.get("id") is not None else None,
        "title": (
            (matched.get("title") if matched else source_position.get("title")) or ""
        ).strip(),
        "orarole": (
            (matched.get("orarole") if matched else source_position.get("orarole")) or ""
        ).strip(),
    }


def _build_smstaff_success_message(
    *,
    db: str,
    auser: str,
    acheck: int,
    ...
    is_block_mode: bool,
    is_delete_mode: bool,
    updated_smstaff_rows: int,
    used_password: str,
) -> str:
    if is_block_mode:
        return (
            f"Для пользователя {auser} в базе {db} выполнена блокировка "
    ...
    if is_delete_mode:
        return (
            f"Процедура выполнена для логина {auser} в базе {db}. "
            f"Режим ACHECK=2."
        )

    if exact_exists and acheck == 1:
        return (
            f"Для существующего пользователя {auser} в базе {db} выполнено обновление. "
            f"Пароль: {used_password}. "
            f"Принудительно обновлено строк в SMSTAFF: {updated_smstaff_rows}."
        )
    ...
    if exact_exists and acheck == 0:
        return (
            f"Пользователь {auser} уже существовал в базе {db}. "
...
def sm_staff_ui_create(request):
    ...
    Новое:
    - можно выбрать сразу несколько баз;
    - можно передать APASS вручную;
    - если APASS пустой, пароль генерируется автоматически;
    - должность выбирается по "базе-источнику", затем сопоставляется в каждой целевой базе.
    """
    services = _ui_services_list()
    ...
    if request.method == "POST":
        ...
        position_source_db = (request.POST.get("position_source_db") or "").strip().upper()
    else:
        raw_selected_dbs = request.GET.getlist("dbs")
        fallback_db = (request.GET.get("db") or "").strip().upper()
        position_source_db = (request.GET.get("position_source_db") or "").strip().upper()
    ...
    if not position_source_db or position_source_db not in selected_dbs:
        position_source_db = selected_dbs[0]

    # Для совместимости со старым шаблоном/ссылками
    db = position_source_db
    ...
    # Загружаем должности из базы-источника
    src_conn = src_cur = None
    try:
        if not _is_allowed_service(position_source_db) or position_source_db not in ORACLE_TNS_MAP:
            raise ValueError(f"Неизвестная база position_source_db={position_source_db!r}.")

        src_conn = _connect_oracle_service(position_source_db)
        src_cur = src_conn.cursor()
        positions = _oracle_fetch_smoffcfg_choices(src_cur)
        positions_map = {str(x["id"]): x for x in positions}
    except Exception as e:
        logger.exception(f"[UI/SMSTAFF_CREATE] load positions error db={position_source_db}: {e}")
        error = str(e)
    finally:
        try:
            if src_cur:
    ...
    if request.method == "POST" and not error:
        ...
        if not error:
            selected_source_pos = positions_map.get(adol_id) if adol_id else None

            source_adol_title = (selected_source_pos.get("title") or "").strip() if selected_source_pos else ""
            source_adol_orarole = (selected_source_pos.get("orarole") or "").strip() if selected_source_pos else None

            if is_creation_mode and selected_source_pos and not source_adol_orarole:
                error = (
                    f"Для выбранной должности "
                    f"'{source_adol_title or selected_source_pos.get('id')}' "
        ...
        if not error:
            ...
            if not re.fullmatch(r"[A-Za-z0-9_.$]+", proc_name):
                error = "Некорректное имя процедуры в SM_BIN_CREATEUSER_PROC."
        ...
        if not error:
            for target_db in selected_dbs:
                ...
                try:
                    ...
                    selected_pos_for_db = None
                    adol_offindex = None
                    adol_title = ""
                    proc_adol = None

                    if selected_source_pos:
                        if target_db == position_source_db:
                            selected_pos_for_db = {
                                "matched": True,
                                "id": int(selected_source_pos["id"]) if selected_source_pos.get("id") is not None else None,
                                "title": (selected_source_pos.get("title") or "").strip(),
                                "orarole": (selected_source_pos.get("orarole") or "").strip(),
                            }
                        else:
                            selected_pos_for_db = _oracle_resolve_position_for_target_db(cur, selected_source_pos)

                        if (
                            selected_source_pos
                            and not is_block_mode
                            and not is_delete_mode
                            and not (selected_pos_for_db and selected_pos_for_db.get("matched"))
                        ):
                            raise ValueError(
                                f"В базе {target_db} не найдена должность, соответствующая "
                                f"выбранной позиции из базы {position_source_db}. "
                                f"Сопоставление идёт по ORAROLE, затем по ID, затем по TITLE."
                            )

                        proc_adol = (selected_pos_for_db.get("orarole") or "").strip() or None if selected_pos_for_db else None
                        adol_offindex = selected_pos_for_db.get("id") if selected_pos_for_db else None
                        adol_title = (selected_pos_for_db.get("title") or "").strip() if selected_pos_for_db else ""
                    ...
                    if is_creation_mode:
                        ...
                        for row in result_item["conflicts"]:
                        ...
                        if login_taken_by_other:
                            raise ValueError(
                                f"В базе {target_db} уже есть пользователь с логином {auser}, "
                                f"но с другим ИНН. Проверь данные."
                            )

                        if inn_taken_by_other:
                            raise ValueError(
                                f"В базе {target_db} уже есть пользователь с ИНН {ainn}, "
                    ...
                    logger.info(
                        f"[UI/SMSTAFF_CREATE] start db={target_db} login={auser} inn={proc_ainn!r} "
                        ...
                        f"is_creation_mode={is_creation_mode} password_mode={'manual' if manual_password else 'auto'}"
                    )
                    ...
                    result_item["success_message"] = _build_smstaff_success_message(
                        db=target_db,
                        auser=auser,
                        acheck=acheck,
                        ...
                        updated_smstaff_rows=updated_smstaff_rows,
                        used_password=created_password,
                    )

                    logger.info(
                        f"[UI/SMSTAFF_CREATE] success db={target_db} login={auser} "
                        f"created_rows={len(result_item['created_rows'])} offindex={adol_offindex} "
                    ...
                    logger.exception(f"[UI/SMSTAFF_CREATE] error db={target_db}: {e}")
    ...
    return render(request, "frostapp/smstaff_create.html", {
        ...
        "position_source_db": position_source_db,
        "form": form,
        "positions": positions,
...
def handle_callback_event(user: User, payload: str, max_user_id: int, max_chat_id: Optional[int], raw_data: dict):
    if payload.startswith("checklist:"):
        return handle_checklist_callback_event(user, payload, max_user_id, max_chat_id, raw_data)
...
def handle_message_event(user: User, text: str, max_user_id: int, max_chat_id: Optional[int], raw_data: dict):
    ...
    checklist_result = handle_checklist_message_event(user, text, max_user_id, max_chat_id, raw_data)
...
def maxbot_process_update(request):
    ...
    try:
        ...
        if event_type == "photo_message":
            checklist_session = get_open_checklist_session(max_user_id)
            if checklist_session and checklist_session.status == "awaiting_photo":
                return JsonResponse(save_checklist_photo_answers(checklist_session, photo_urls, raw_payload=raw_update))
...
def get_user_checklist_departments(user: User):
    if not user:
        return MaxBotChecklistDepartment.objects.none()

    return (
        MaxBotChecklistDepartment.objects
        .filter(
            is_active=True,
            access_rows__is_active=True,
            access_rows__user_department_id=user.department_id,
        )
        .distinct()
        .order_by("sort_order", "name")
    )
...
def user_has_checklist_access(user: User) -> bool:
    return get_user_checklist_departments(user).exists()


def get_open_checklist_session(max_user_id: int) -> Optional[MaxBotChecklistSession]:
    return (
        MaxBotChecklistSession.objects
        .filter(
            max_user_id=max_user_id,
            status__in=[
                "awaiting_department",
...
def cancel_previous_open_checklist_sessions(max_user_id: int):
    MaxBotChecklistSession.objects.filter(
        max_user_id=max_user_id,
        status__in=[
            "awaiting_department",
            "awaiting_location",
...
def create_new_checklist_session_for_user(user: User, max_user_id: int, max_chat_id: Optional[int], raw_event=None):
    cancel_previous_open_checklist_sessions(max_user_id)
    cancel_previous_open_requests(max_user_id)

    return MaxBotChecklistSession.objects.create(
        session_no=generate_checklist_session_no(),
        applicant_user=user,
        applicant_full_name=user.full_name or "",
        max_user_id=max_user_id,
        max_chat_id=max_chat_id,
...
def _cancel_checklist_session(max_user_id: int):
...
def ask_checklist_department(session: MaxBotChecklistSession):
    departments = list(get_user_checklist_departments(session.applicant_user))
    if not departments:
        return build_response(
            text="Для вашего department_id новая ветка чек-листа не настроена.",
            buttons=[[kb_message("Начать")]],
        )
...
def ask_checklist_location(session: MaxBotChecklistSession):
    locations = list(
        MaxBotChecklistLocation.objects
        .filter(checklist_department=session.checklist_department, is_active=True)
        .order_by("sort_order", "name")
...
def ask_checklist_workplace(session: MaxBotChecklistSession):
    workplaces = list(
        MaxBotChecklistWorkplace.objects
        .filter(location=session.location, is_active=True)
        .order_by("sort_order", "name")
    ...
    return build_response(
        text=(
            f"Подразделение: <b>{escape(session.department_name)}</b>\n"
            f"Месторасположение: <b>{escape(session.location_name)}</b>\n\n"
...
def get_first_checklist_question() -> Optional[MaxBotChecklistQuestion]:
    return (
        MaxBotChecklistQuestion.objects
...
def get_next_checklist_question(question: MaxBotChecklistQuestion) -> Optional[MaxBotChecklistQuestion]:
    return (
        MaxBotChecklistQuestion.objects
        .filter(is_active=True, sort_order__gt=question.sort_order)
        .order_by("sort_order", "id")
        .first()
    )
...
def ask_checklist_question(session: MaxBotChecklistSession, question: MaxBotChecklistQuestion, notification: Optional[str] = None):
    ...
    text = (
        f"<b>Подразделение:</b> {escape(session.department_name)}\n"
        f"<b>Месторасположение:</b> {escape(session.location_name)}\n"
        f"<b>Рабочее место:</b> {escape(session.workplace_name)}\n"
        f"<b>Ответственный:</b> {escape(session.responsible_name or '-')}\n\n"
        f"<b>Шаг {step_index}/5 — {escape(question.step_name)}</b>\n"
        f"<b>Вопрос {question.question_no}/6</b>\n\n"
...
def _get_pending_checklist_answer(session: MaxBotChecklistSession) -> Optional[MaxBotChecklistAnswer]:
    if not session or not session.current_question_id:
        return None

    return (
        session.answers
        .filter(question_id=session.current_question_id)
        .order_by("-id")
        .first()
    )
...
def save_checklist_boolean_answer(session: MaxBotChecklistSession, question: MaxBotChecklistQuestion, answer_value: bool, raw_payload=None):
    if not session.current_question_id:
        return build_response(text="Нет активного вопроса.", buttons=[[kb_message("Начать")]])

    if session.current_question_id != question.id:
        return ask_checklist_question(
            session,
            session.current_question,
            notification="Этот вопрос уже не актуален. Ответьте ещё раз.",
        )

    score = question.yes_score if answer_value else question.no_score
    photo_required = question.photo_required_if_yes if answer_value else question.photo_required_if_no
    answer_label = "Да" if answer_value else "Нет"

    MaxBotChecklistAnswer.objects.create(
        session=session,
        question=question,
        step_code=question.step_code,
        step_name=question.step_name,
    ...
    if photo_required:
        ...
        session.save(update_fields=["status", "raw_last_event", "updated_at"])

        _ensure_checklist_report_dir(session)

        return build_response(
            text=(
                f"Ответ <b>{answer_label}</b> сохранён.\n\n"
                "По этому вопросу обязательно приложите одно или несколько фото, "
                "затем нажмите <b>«Продолжить»</b>."
            ),
            buttons=_photo_step_buttons(),
        )
    ...
    next_question = get_next_checklist_question(question)
    if next_question:
        return ask_checklist_question(session, next_question, notification="Ответ сохранён.")

    return finish_checklist_session(session, notification="Ответ записан.")




def _download_and_save_checklist_photo(
    session: MaxBotChecklistSession,
    answer: MaxBotChecklistAnswer,
    photo_obj: MaxBotChecklistAnswerPhoto,
    seq_no: int,
):
    report_dir = _ensure_checklist_report_dir(session)

    if not photo_obj.photo_url:
        return

    try:
        resp = requests.get(photo_obj.photo_url, timeout=30)
        resp.raise_for_status()
        ...
        photo_obj.save(update_fields=["photo_file", "saved_name", "original_name"])

    except Exception:
        logger.exception("Не удалось скачать фото для чек-листа: %s", photo_obj.photo_url)


def save_checklist_photo_answers(session: MaxBotChecklistSession, photo_urls: list[str], raw_payload=None):
    ...
    created_count = 0
    existing_count = pending.photos.count()

    for i, photo_url in enumerate([u for u in (photo_urls or []) if u], start=existing_count + 1):
    ...
    return build_response(
        text=(
            f"Фото сохранено: <b>+{created_count}</b>\n"
            f"Всего фото по этому вопросу: <b>{total_count}</b>\n\n"
            "Можно отправить ещё фото или нажать <b>«Продолжить»</b>."
        ),
        buttons=_photo_step_buttons(),
    )
...
def complete_checklist_photo_step(session: MaxBotChecklistSession):
    pending = _get_pending_checklist_answer(session)
    if not pending:
        return build_response(text="Нет активного фото-шагa.", buttons=[[kb_message("Начать")]])

    if pending.photo_required and not pending.photos.exists():
        return build_response(
            text="Сначала отправьте хотя бы одно фото.",
            buttons=_photo_step_buttons(),
        )

    next_question = get_next_checklist_question(session.current_question)
    if next_question:
        return ask_checklist_question(session, next_question, notification="Фото сохранены.")
...
def build_checklist_summary(session: MaxBotChecklistSession) -> tuple[int, str]:
    ...
    for ans in answers:
        ...
        lines.append(f"{ans.step_name} / {ans.question_no}: {ans.answer_label} ({ans.score})")
...
def finish_checklist_session(session: MaxBotChecklistSession, notification: Optional[str] = None):
    total_score, summary = build_checklist_summary(session)
    report_file = generate_checklist_excel_report(session)
...
def handle_checklist_callback_event(user: User, payload: str, max_user_id: int, max_chat_id: Optional[int], raw_data: dict):
    payload = (payload or "").strip()

    if payload == "checklist:start":
        if not user_has_checklist_access(user):
            return build_main_menu(user)
        session = create_new_checklist_session_for_user(user, max_user_id, max_chat_id, raw_event=raw_data)
        return ask_checklist_department(session)

    if payload == "checklist:cancel":
    ...
    if payload.startswith("checklist:department:"):
        department_id = int(payload.split(":")[2])
        department = (
            get_user_checklist_departments(user)
    ...
    if payload.startswith("checklist:location:"):
        location_id = int(payload.split(":")[2])
        location = MaxBotChecklistLocation.objects.filter(
            id=location_id,
            checklist_department=session.checklist_department,
            is_active=True,
        ).first()
        if not location:
            return ask_checklist_location(session)
    ...
    if payload.startswith("checklist:workplace:"):
        workplace_id = int(payload.split(":")[2])
        workplace = MaxBotChecklistWorkplace.objects.filter(
            id=workplace_id,
    ...
    if payload.startswith("checklist:answer:"):
        _, _, answer_raw, question_id_raw = payload.split(":")
        answer_value = bool(int(answer_raw))
        question_id = int(question_id_raw)

        question = MaxBotChecklistQuestion.objects.filter(id=question_id, is_active=True).first()
        if not question:
            return build_response(text="Вопрос не найден.", buttons=[[kb_message("Начать")]])

        return save_checklist_boolean_answer(session, question, answer_value, raw_payload=raw_data)

    return build_main_menu(user)


def handle_checklist_message_event(user: User, text: str, max_user_id: int, max_chat_id: Optional[int], raw_data: dict):
    cleaned = (text or "").strip()
    normalized = _normalize_choice_text(cleaned)

    if normalized in {"7с чек-лист", "7s чек-лист", "чек-лист", "7с"}:
        if not user_has_checklist_access(user):
            return build_main_menu(user)
        session = create_new_checklist_session_for_user(user, max_user_id, max_chat_id, raw_event=raw_data)
        return ask_checklist_department(session)
